...
const breadcrumbItems = ref([
  ...
  { name: `配置详情 (ID: ${configId.value})`, path: "" }, // 当前页不需要跳转
]);

watch(configId, (newId: any) => {
  breadcrumbItems.value = [
    { name: "首页", path: "/" },
    { name: "配置列表", path: "/" },
    { name: `配置详情 (ID: ${newId})`, path: "" },
  ];
});
...
const m3uUrl = "https://raw.githubusercontent.com/fanmingming/live/main/tv/m3u/ipv6.m3u";

// 将result作为source存入sources中
m3uStore.addSource(m3uUrl, result);
console.log("sources: ", m3uStore.sources);

const play = (src: string) => {
  openPlayerWindow({ src, m3uSourceUrl: m3uUrl });
};
